...
def sanitizeHtml(value, base_url=None, validTags="p i strong b u a h1 h2 h3 pre br img", validAttrs="href src width height"):
    rjs = r'[\s]*(&#x.{1,7})?'.join(list('javascript:'))
    rvb = r'[\s]*(&#x.{1,7})?'.join(list('vbscript:'))
    re_scripts = re.compile('(%s)|(%s)' % (rjs, rvb), re.IGNORECASE)
    validTags = validTags.split()
    validAttrs = validAttrs.split()
...
def commasplit(s, numeric=False):
    if s:
        li = s.split(',')
        if len(li):
            if not li[-1]:
...
def barJoin(li):
    return "|%s|" % "|".join(str(x) for x in li)
